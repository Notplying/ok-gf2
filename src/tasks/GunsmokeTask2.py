...
class GunsmokeTask2(BaseGfTask):
    """
    Task that navigates to Platoon → Gunsmoke → Ranking, scrolls through the
    ranking list, and takes screenshots of all entries.

    This is a second instance of GunsmokeTask to make scheduling easier —
    you can run two gunsmoke ranking screenshot tasks independently.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.name = "Gunsmoke Ranking Screenshot 2"
        self.description = (
            "Navigates to Platoon → Gunsmoke → Ranking, scrolls the ranking "
            "list, and screenshots all entries. (Second instance). Game must be running (logged in)."
        )
    ...
    def run(self):
        ...
        """
        self.log_info("Starting Gunsmoke Ranking Screenshot 2 task")
        ...
        self.log_info("Gunsmoke Ranking Screenshot 2 task completed", notify=True)
        return True
        ...
        try:
            if captured_paths:
                ...
                DiscordNotifier.send_screenshots(
                    captured_paths, "Gunsmoke Ranking 2", content="gunsmoke"
                )
